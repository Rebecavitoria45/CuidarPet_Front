...
export default function GerenciarAgendamentos() {
            ...
            <thead>
              <tr className="bg-gray-50 border-b border-outline-variant">
                ...
                <th className="p-4 w-52">
                  <span className="text-xs font-bold text-on-surface-variant uppercase ml-1">Data (De/Até)</span>
                ...
                {/* PACIENTE - Dropdown dinâmico */}
                <th className="p-4 w-64 relative">
                  <span className="text-xs font-bold text-on-surface-variant uppercase ml-1">Paciente</span>
                  <div className="relative mt-2">
                    <input
                      type="text"
                      placeholder={petSelecionadoFiltro ? `Filtrando: ${petSelecionadoFiltro.nome}` : "Nome do pet..."}
                      className={`${inputFilterClass} h-[42px] ${petSelecionadoFiltro ? 'bg-orange-50 border-orange-300 font-bold' : ''}`}
                      value={termoPetFiltro}
                      onChange={(e) => {
                        setTermoPetFiltro(e.target.value);
                        if (petSelecionadoFiltro) setPetSelecionadoFiltro(null);
                      }}
                    />
                    {mostrarDropdownFiltro && (
                      <div className="absolute z-[110] w-full mt-1 bg-white border border-outline-variant rounded-xl shadow-2xl max-h-48 overflow-y-auto">
                        {petsSugeridos.map(p => (
                          <button key={p.id} type="button" onClick={() => {
                            setPetSelecionadoFiltro(p);
                            setTermoPetFiltro(p.nome);
                            setMostrarDropdownFiltro(false);
                          }} className="w-full text-left px-4 py-3 hover:bg-orange-50 border-b border-gray-50 flex flex-col transition-colors">
                            <span className="text-sm font-bold text-on-surface">{p.nome}</span>
                            <span className="text-[10px] text-gray-400 uppercase font-medium">Tutor: {p.tutorNome}</span>
                          </button>
                        ))}
                      </div>
                    )}
                ...
                <th className="p-4">
                  ...
                  <select className={`${inputFilterClass} mt-2 h-[42px]`} value={filtros.vet} onChange={(e) => setFiltros({...filtros, vet: e.target.value})}>
                ...
                <th className="p-4 w-40">
                  ...
                  <select className={`${inputFilterClass} mt-2 h-[42px]`} value={filtros.status} onChange={(e) => setFiltros({...filtros, status: e.target.value})}>
                    <option value="Todos">Todos</option>
                ...
                {/* AÇÕES*/}
                <th className="p-4 w-40 text-center align-bottom">
                   <div className="flex flex-col gap-2">
                      <button onClick={aplicarFiltros} className="w-full h-10 bg-primary-container text-white text-xs font-bold rounded-lg hover:bg-orange-600 transition shadow-md flex items-center justify-center gap-2 active:scale-95">
                        <span className="material-symbols-outlined text-lg">search</span> Buscar
                      </button>
                      <button onClick={limparFiltros} className="w-full h-10 border border-gray-200 text-gray-500 text-xs font-bold rounded-lg hover:bg-gray-50 transition active:scale-95">
                        Limpar
                      </button>
                   </div>
                </th>
  ...
  <div className="flex justify-center gap-1">
    ...
    {/* VISÃO DO VETERINÁRIO: Apenas o botão Concluir */}
    {isVeterinario && !isAdmin ? (
      <>
        {ag.status !== 'CONCLUIDO' && ag.status !== 'CANCELADO' && (
          <button
            onClick={() => abrirModal(ag, 'CONCLUIR')}
            className="flex items-center gap-1 px-3 py-1.5 bg-emerald-600 text-white rounded-lg font-bold hover:bg-emerald-700 transition-all active:scale-95 shadow-sm"
            title="Concluir Atendimento"
          >
            <span className="material-symbols-outlined text-[18px]">task_alt</span>
            <span className="text-[10px] uppercase">Concluir</span>
          </button>
        )}
        {/* Se já estiver concluído, mostra apenas um informativo */}
        {ag.status === 'CONCLUIDO' && (
           <span className="text-[10px] text-gray-400 font-bold uppercase italic">Atendimento Finalizado</span>
        )}
      </>
    ) : (
      /* VISÃO ADMIN / ATENDENTE */
      <>
        {ag.status !== 'CONFIRMADO' && ag.status !== 'CONCLUIDO' && (
          <button onClick={() => abrirModal(ag, 'CONFIRMAR')} className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-full transition-colors">
            <span className="material-symbols-outlined text-[20px]">check_circle</span>
          </button>
        )}
        {ag.status !== 'CANCELADO' && ag.status !== 'CONCLUIDO' && (
          <button onClick={() => abrirModal(ag, 'CANCELAR')} className="p-2 text-red-600 hover:bg-red-50 rounded-full transition-colors">
            <span className="material-symbols-outlined text-[20px]">cancel</span>
          </button>
        )}
        <button onClick={() => navigate(`/agendamentos/editar/${ag.id}`)} className="p-2 text-gray-400 hover:text-primary transition-colors">
