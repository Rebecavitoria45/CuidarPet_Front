...
export default function GerenciarAgendamentos() {
  ...
  const carregarDados = useCallback(async () => {
    try {
      ...
      setVeterinarios(resVets.data.filter(u => u.role === 'VETERINARIO'));
    } catch (error) {
  ...
  const handleTrocarStatus = async () => {
    ...
    // Filtro de texto (Pet ou Veterinário)
    const bateTexto = ag.petNome.toLowerCase().includes(filtroTexto.toLowerCase()) || 
                     ag.veterinarioNome.toLowerCase().includes(filtroTexto.toLowerCase());
    
    // Filtro de Status
    const bateStatus = filtroStatus === 'Todos' || ag.status === filtroStatus;
    
    // Filtro de Veterinário (Select)
    const bateVet = filtroVet === 'Todos' || ag.veterinarioNome === filtroVet;

    // Filtro de Data 
    // Pegamos apenas a parte YYYY-MM-DD da data que vem do banco
    const dataBancoPura = ag.data.substring(0, 10); 
    
    // Compara string com string ("2026-06-21" === "2026-06-21")
    const bateData = !filtroData || dataBancoPura === filtroData;


    // Retorna true apenas se passar em TODOS os filtros ao mesmo tempo
    return bateTexto && bateStatus && bateVet && bateData;
});


  // Estilo comum para os inputs de filtro no header
  const inputFilterClass = "w-full mt-2 p-1.5 text-[11px] font-normal border border-gray-200 rounded-md focus:ring-1 focus:ring-primary-container outline-none bg-white text-on-surface";
  ...
  return (
    <Layout>
      <div className="w-full animate-entrance">
        ...
        <div className="bg-white border border-outline-variant rounded-2xl overflow-hidden shadow-sm">
          <table className="w-full text-left border-collapse table-fixed">
            <thead>
              <tr className="bg-gray-50 border-b border-outline-variant">
                ...
                {/* Coluna Horário */}
                <th className="p-4 w-28 align-top">
                  <span className="text-xs font-bold text-on-surface-variant uppercase">Horário</span>
                </th>

                {/* Coluna Pet (Filtro por busca rápida) */}
                <th className="p-4">
                  <span className="text-xs font-bold text-on-surface-variant uppercase ml-1">Paciente / Pet</span>
                  <input 
                    type="text" 
                    placeholder="Filtrar..."
                    className={inputFilterClass}
            ...
            <tbody className="divide-y divide-outline-variant">
              {agendamentosFiltrados.map((ag) => (
                <tr key={ag.id} className={`hover:bg-orange-50/20 transition-colors ${ag.status === 'CANCELADO' ? 'opacity-60 grayscale-[0.5]' : ''}`}>
                  ...
                  <td className="p-4">
                    <div className="flex flex-col">
                      <span className="font-bold text-on-surface">{ag.petNome}</span>
                    </div>
                  </td>
                  ...
                  <td className="p-4">
                    <div className="flex justify-center gap-1">
                      {ag.status !== 'CANCELADO' && ag.status !== 'CONCLUIDO' && (
                        <>
                          <button 
                            ...
                            className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-full transition-colors" title="Confirmar Presença"
                          >
                            <span className="material-symbols-outlined text-[20px]">check_circle</span>
                          </button>
                          <button 
                            onClick={() => abrirModal(ag, 'CANCELAR')}
                            className="p-1.5 text-red-600 hover:bg-red-50 rounded-full transition-colors" title="Cancelar"
                          >
                      ...
                      <button 
                        ...
                        className="p-1.5 text-gray-400 hover:text-primary transition-colors" title="Editar"
                      >
          ...
          {agendamentosFiltrados.length === 0 && !loading && (
            <div className="p-12 text-center text-on-surface-variant italic bg-gray-50/50">
      ...
      {/* Modal de Confirmação mantém a mesma lógica anterior */}
      {isModalOpen && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 animate-entrance border border-orange-100">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-on-surface-variant text-sm mb-8 leading-relaxed">
                Deseja alterar o status do agendamento de <span className="font-bold text-on-surface">{agendamentoSelecionado?.petNome}</span> para 
              ...
              <div className="flex gap-4 w-full">
                ...
                <button 
                  ...
                  className={`flex-1 py-3 text-white rounded-xl font-black uppercase tracking-wider shadow-lg active:scale-95 transition-all ${acaoModal === 'CONFIRMAR' ? 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-100' : 'bg-red-600 hover:bg-red-700 shadow-red-100'}`}
                >
                  {acaoModal === 'CONFIRMAR' ? 'Confirmar' : 'Cancelar'}
                </button>
              </div>
            </div>
          </div>
