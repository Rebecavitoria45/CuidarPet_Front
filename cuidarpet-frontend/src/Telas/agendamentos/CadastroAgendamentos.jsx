...
export default function CadastrarAgendamento() {
  ...
  useEffect(() => {
    api.get('/usuarios').then(res => {
      const ativos = res.data.filter(u => 
      u.role === 'VETERINARIO' && u.ativo === true
    );
    setVets(ativos);
  });
  }, []);
 ...
 // Busca horários ocupados sempre que mudar o Veterinário ou a Data
  useEffect(() => {
    ...
    // Se o veterinário for desmarcado, limpamos  o horário selecionado
  ...
  }, [formData.veterinarioId, formData.data]);
